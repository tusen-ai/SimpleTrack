...
# paths
parser.add_argument('--result_name', type=str, default='gt_output')
...
def sequence_assign(pd_bboxes, pd_ids, pd_states, gt_bboxes, gt_ids):
    ...
    for frame_index in range(frame_num):
        ...
        index = list(reversed(sorted(range(len(frame_pds)), key=lambda k:frame_pds[k].s)))
        ...
        frame_result_states = list()
        frame_result_pds, frame_result_ids = [], []
        selected_ids = list()
        for i in range(len(frame_pds)):
            if max_iou[i] > args.thres and max_index[i] not in selected_ids:
                frame_result_states.append('alive_1_1')
                frame_result_pds.append(frame_gts[max_index[i]])
