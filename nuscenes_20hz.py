...
from data_loader import NuScenesLoader20Hz
...
# parser.add_argument('--obj_types', default='bicycle')
...
def main(name, obj_types, config_path, data_folder, det_data_folder, result_folder, start_frame=0, token=0, process=1):
    for obj_type in obj_types:
        ...
        for file_index, file_name in enumerate(file_names[:]):
            ...
            data_loader = NuScenesLoader20Hz(configs, [obj_type], segment_name, data_folder, det_data_folder, start_frame)
...
if __name__ == '__main__':
    if args.test:
        args.data_folder = os.path.join(args.data_folder, 'test_20hz')
        args.det_data_folder = os.path.join(args.det_data_folder, 'test_20hz', 'detection')
    else:
        args.data_folder = os.path.join(args.data_folder, 'validation_20hz')
        args.det_data_folder = os.path.join(args.det_data_folder, 'validation_20hz', 'detection')
